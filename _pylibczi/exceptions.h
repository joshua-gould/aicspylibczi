...
namespace pylibczi {
    ...
    class FilePtrException : public std::exception {
      std::string m_msg;
    public:
      explicit FilePtrException(std::string str) : exception(), m_msg(std::move(str)) {}
    ...
    public:
      PixelTypeException(libCZI::PixelType pt, std::string msg) : exception(), m_ptype(pt), m_msg(std::move(msg)) {}
    ...
    public:
      ImageAccessUnderspecifiedException(int given, int required, std::string msg)
          : m_given(given), m_required(required), m_msg(msg) {}
    ...
    class ImageIteratorException : public std::exception {
      std::string m_msg;
    public:
      explicit ImageIteratorException(std::string msg) : m_msg("ImageIteratorException: " + msg) {
          int x = 5;
      }
      const char *what() const noexcept override {
          return m_msg.c_str();
